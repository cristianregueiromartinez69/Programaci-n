		...
		for(int i=0;i<array.length;i++) {
			...
			for(int j=0;j<array[i].length;j++) {
				...
				suma+=array[i][j];
                        ...
                        Goles_Equipos.golazo.add(new Goles_Equipos(Equipo.nombres[i], suma));
...
public void ListaEquiposMenosGoles(int[][]array) {
		System.out.println();
		System.out.println("EQUIPOS ORDENADOS POR NUMERO DE GOLES DE MENOR A MAYOR");
		System.out.println();
		Collections.sort(Goles_Equipos.golazo, Comparator.comparing(Goles_Equipos::getSuma));

        for(Goles_Equipos goles: Goles_Equipos.golazo){
